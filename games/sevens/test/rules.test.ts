...
import type { SevensBoardState } from "../shared/types.ts";

const board = (rows: Partial<SevensBoardState>): SevensBoardState => ({ ...emptyBoard(), ...rows });
...
describe("cards", () => {
  ...
  it("placeCard widens the right end", () => {
    let b = placeCard(emptyBoard(), makeCard("hearts", 7));
    b = placeCard(b, makeCard("hearts", 8));
    b = placeCard(b, makeCard("hearts", 6));
