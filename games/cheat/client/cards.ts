...
function faceParts(c: Pick<Card, "suit" | "rank">): Node[] {
...
/** A card face you can't press: reveals, your own play, and animation ghosts. */
export function cardFace(c: Pick<Card, "suit" | "rank">): HTMLElement {
  return h("span", { class: `ch-card${isRed(c.suit) ? " red" : ""}`, role: "img", "aria-label": cardLabel(c) }, faceParts(c));
}
...
export function handCard(c: Card): HTMLButtonElement {
  return h(
    "button",
    { type: "button", class: `ch-card${isRed(c.suit) ? " red" : ""}`, "aria-label": cardLabel(c), "data-card": c.id },
    faceParts(c),
  );
}

/** A full-size card back, for the pile, plays on it, and cards in flight. */
export function cardBackFull(): HTMLElement {
  return h("span", { class: "ch-card back", "aria-hidden": "true" });
}

/** A small card back, for opponents' hand sizes. */
export function cardBackMini(): HTMLElement {
  return h("span", { class: "ch-back-mini", "aria-hidden": "true" });
}
