...
/** A card face you can't press: the pile's top card, and animation ghosts. */
export function cardFace(c: Pick<Card, "suit" | "rank">): HTMLElement {
  return h("span", { class: `ce-card${isRed(c.suit) ? " red" : ""}`, role: "img", "aria-label": cardLabel(c) }, faceParts(c));
}
...
export function handCard(c: Card): HTMLButtonElement {
  return h(
    "button",
    { type: "button", class: `ce-card${isRed(c.suit) ? " red" : ""}`, "aria-label": cardLabel(c), "data-card": c.id },
    faceParts(c),
  );
}

/** A full-size card back, for the stock and for cards in flight. */
export function cardBackFull(): HTMLElement {
  return h("span", { class: "ce-card back", "aria-hidden": "true" });
}

/** A small card back, for opponents' hand sizes. */
export function cardBackMini(): HTMLElement {
  return h("span", { class: "ce-back-mini", "aria-hidden": "true" });
}
